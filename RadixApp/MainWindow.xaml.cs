...
namespace WpfApp1
{
    public partial class MainWindow : UiControls.FluentWindow
    {
        ...
        private void SetupTray()
        {
            ...
            _tray.Text = "SoundCloud Desktop Alt";
        ...
        public void MinimizeToTray()
        {
            try
            {
                ...
                if (_tray != null)
                {
                    _tray.BalloonTipTitle = "SoundCloud Desktop Alt";
        ...
        public void ApplyAutostart()
        {
            try
            {
                    ...
                    @"SOFTWARE\Microsoft\Windows\CurrentVersion\Run", true))
                {
                    if (key == null) return;
                    if (_state.Autostart)
                        key.SetValue("SoundCloudDesktopBetaAlt", "\""
                            + Process.GetCurrentProcess().MainModule.FileName + "\"");
                    else
                        key.DeleteValue("SoundCloudDesktopBetaAlt", false);
                }
            }
            catch { }
        }
